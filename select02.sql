...
select LENGTH('asdoijf'),LENGTHB('asdoijf'),LENGTH('한글'), LENGTHB('한글') --한글 한글자 3byte 
from dual;

--이름이 알파벳 5자 이상만 조회
select * from emp where LENGTH(ename)>=5;

select '아침'||'점심', CONCAT('아침','점심') from dual;

select '아침'||'점심'||'저녁',concat(CONCAT('아침','점심'),'저녁') from dual;
...
select
    ...
    CEIL(rownum/4) 팀구분,     --4/3 = 1.33 -> 2 5/3 = 1.66 ->  2 6/3 = 2
    empno,
    ename
from emp;
--where rownum <=5;
