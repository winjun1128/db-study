--1
select ename,replace(ename,substr(ename,3,2),'--') REPLACE from emp where deptno=20;
--2
select name,replace(jumin,substr(jumin,7,7),'-/-/-/-') 주민뒷자리숨김 from student where deptno1=101;
--3
select name,tel,replace(tel,substr(tel,1,3),'***') REPLACE from student where deptno1=102;
--3-옵션 풀었음
select name,tel,RPAD(SUBSTR(tel,1,INSTR(tel,')')),INSTR(tel,'-')-1,'*')||SUBSTR(tel,-5,5) REPLACE FROM student;
--4
select name,tel,replace(tel,substr(tel,-4),'****') REPLACE from student where deptno1=101;
/**********************************************/
--1.
...
select
    ...
    SUBSTR(tel,1,INSTR(SUBSTR(tel, INSTR(tel, ')') + 1), '-') - 1),
    REPLACE(tel,SUBSTR(tel,INSTR(tel,')')+1,(INSTR(tel,'-')-INSTR(tel,')')-1))-1),
