--1
select name 이름, SUBSTR(birthday,1,2) 년도, SUBSTR(birthday,4,2) 월, SUBSTR(birthday,7,2) 일 from student;
--2
select name,tel,INSTR(tel,')') 괄호위치 from student where deptno1=201;
--3
select name,tel,INSTR(tel,'3') 괄호위치 from student where deptno1=101;
--4
select name,tel,substr(tel,1,INSTR(tel,')')-1) 지역번호 from student where deptno1=201;
...
select 
    ...
    SUBSTR(jumin,5,2) 일
    --SUBSTR(birthday,1,2),
    --SUBSTR(birthday,4,2) from student
from student;
--2.
