...
--1.
DELETE FROM 테이블명;       -> 데이터만 삭제
TRUNCATE TABLE 테이블명;    -> 테이블을 삭제

--1. 다음 두 명령어는 어떤 기능을 수행하는 명령어인지 작성하고,
--두 기능의 차이점이 있다면 설명하시오.
DELETE FROM 테이블명;         DML 데이터 삭제 조건으로 원하는 데이터만 삭제(WHERE 특정 데이터 선택) commit/rollback
TRUNCATE TABLE 테이블명;      DDL 데이터 삭제 전체 데이터 바로 삭제   삭제시 바로 적용(auto commit)
...
INSERT INTO T_MEMBER_POINT VALUES( T_MEMBER_POINT_PK_SEQ.nextval,NVL(SELECT * from T_MEMBER WHERE mem_id = '멤버아이디',1) '멤버아이디',100,SYSDATE)
