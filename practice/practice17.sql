...
select
    e2.name,
    e2.position,
    TO_CHAR(e2.pay,'99,999,999,999') pay,
    p.s_pay lowpay,
    p.e_pay highpay
...
select
    e2.name,
    TO_NUMBER(TO_CHAR(e2.birthday,'YY'))-TO_NUMBER(TO_CHAR(TO_DATE('2010-05-30', 'YYYY-MM-DD'), 'YY'))+1 한국나이,
    e2.position 지금포지션,
    CASE
        WHEN TO_NUMBER(TO_CHAR(e2.birthday,'YY'))-TO_NUMBER(TO_CHAR(TO_DATE('2010-05-30', 'YYYY-MM-DD'), 'YY'))+1 BETWEEN p.s_age AND p.e_age THEN p.position
    END 그나이에맞는포지션
from emp2 e2,p_grade p
where e2.position=p.position(+);
--4
...
select
    ...
    pf.name,
    pf.hiredate,
    count(pf.hiredate<pf2.hiredate)
from professor pf, professor pf2;
...
select profno,(select name where profno =....), (select hiredate where profno = ...), count
FROM(
